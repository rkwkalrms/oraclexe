...
-- Q2. "jobs" 테이블에서 모든 직무들의 직무 ID(job_title)을 조회하세요.
SELECT job_id, job_title
FROM jobs;

-- Q3. "department" 테이블에서 모든 부서들의 부서 ID(department_id)와 부서명(department_name)을 조회하세요.
SELECT department_id, department_name
FROM departments;

-- Q4. "location" 테이블에서 모든 지역들의 지역 ID(location_id)와 도시(city)를 조회하세요.
